...
#
# LAST MEASURED: NET-003 (RED), which added six test files (tests/helpers/
...
# `selene over 78 files`, `79` with the untracked probe present - and the
# narrow assertions did not move; `analyze over 15 files` is unchanged
# because nothing was added under src.
...
BASE_FORMAT=78     # stylua  over src tests lune   (78 = 15 src + 63 tests/lune)
BASE_LINT=78       # selene  over src tests lune
BASE_TYPECHECK=15  # analyze over src (NET-001 probe included; src/net/ is three modules now)
NARROW_FORMAT=15   # stylua  over src alone
NARROW_LINT=15     # selene  over src alone
...
assert_observed "format reports 78 files on the unmodified tree"    "$BASE_FORMAT" "$OUT" "$EV_FORMAT"

run_conf_cmd "$CMD_LINT"
assert_zero     "the lint gate passes on the unmodified tree"       "$RC" "$OUT"
assert_observed "lint reports 78 files on the unmodified tree"      "$BASE_LINT" "$OUT" "$EV_LINT"

run_conf_cmd "$CMD_TYPECHECK"
assert_zero     "the typecheck gate passes on the unmodified tree"  "$RC" "$OUT"
assert_observed "typecheck reports 15 files on the unmodified tree"  "$BASE_TYPECHECK" "$OUT" "$EV_TYPECHECK"
...
assert_narrowed_count "narrowing the format target to src reports 15, not 78" \
...
assert_narrowed_count "narrowing the lint target to src reports 15, not 78" \
...
assert_observed "AC-2: format counts the untracked file (78 -> 79)" "$((BASE_FORMAT + 1))" "$OUT" "$EV_FORMAT"

run_conf_cmd "$CMD_LINT"
assert_zero     "AC-4: the lint gate still passes with the untracked file present"   "$RC" "$OUT"
assert_observed "AC-4: lint counts the untracked file (78 -> 79)"   "$((BASE_LINT + 1))" "$OUT" "$EV_LINT"
...
assert_observed "typecheck counts the untracked file (15 -> 16)"      "$((BASE_TYPECHECK + 1))" "$OUT" "$EV_TYPECHECK"
...
assert_observed "format does not count the ignored file (still 78)"    "$BASE_FORMAT" "$OUT" "$EV_FORMAT"

run_conf_cmd "$CMD_LINT"
assert_zero     "the lint gate still passes with an ignored .luau file present"      "$RC" "$OUT"
assert_observed "lint does not count the ignored file (still 78)"      "$BASE_LINT" "$OUT" "$EV_LINT"

run_conf_cmd "$CMD_TYPECHECK"
assert_zero     "the typecheck gate still passes with an ignored .luau file present" "$RC" "$OUT"
assert_observed "typecheck does not count the ignored file (still 15)"  "$BASE_TYPECHECK" "$OUT" "$EV_TYPECHECK"
