...
assert_observed "format reports 53 files on the unmodified tree"    "$BASE_FORMAT" "$OUT" "$EV_FORMAT"

run_conf_cmd "$CMD_LINT"
assert_zero     "the lint gate passes on the unmodified tree"       "$RC" "$OUT"
assert_observed "lint reports 53 files on the unmodified tree"      "$BASE_LINT" "$OUT" "$EV_LINT"
...
assert_narrowed_count "narrowing the format target to src reports 9, not 53" \
...
assert_narrowed_count "narrowing the lint target to src reports 9, not 53" \
...
assert_observed "AC-2: format counts the untracked file (53 -> 54)" "$((BASE_FORMAT + 1))" "$OUT" "$EV_FORMAT"

run_conf_cmd "$CMD_LINT"
assert_zero     "AC-4: the lint gate still passes with the untracked file present"   "$RC" "$OUT"
assert_observed "AC-4: lint counts the untracked file (53 -> 54)"   "$((BASE_LINT + 1))" "$OUT" "$EV_LINT"
...
assert_observed "format does not count the ignored file (still 53)"    "$BASE_FORMAT" "$OUT" "$EV_FORMAT"

run_conf_cmd "$CMD_LINT"
assert_zero     "the lint gate still passes with an ignored .luau file present"      "$RC" "$OUT"
assert_observed "lint does not count the ignored file (still 53)"      "$BASE_LINT" "$OUT" "$EV_LINT"
