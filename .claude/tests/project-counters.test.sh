...
#
# LAST MEASURED: SEAT-002 (RED), which added four test files and no source:
# 43/43/8 -> 47/47/8, narrow unchanged at 8/8/5. GREEN adds
# src/server/seats/Projection.luau, which moves BASE_FORMAT/BASE_LINT to 48,
# BASE_TYPECHECK to 9 and NARROW_FORMAT/NARROW_LINT to 9 - a harness write,
# permitted in GREEN, recorded in that story's `## Regressions`.
...
BASE_FORMAT=47     # stylua  over src tests lune   (47 = 8 src + 39 tests/lune)
BASE_LINT=47       # selene  over src tests lune
BASE_TYPECHECK=8   # analyze over src
NARROW_FORMAT=8    # stylua  over src alone
NARROW_LINT=8      # selene  over src alone
NARROW_TYPECHECK=5 # analyze over src/shared alone (unchanged by SEAT-001)
...
assert_observed "format reports 47 files on the unmodified tree"    "$BASE_FORMAT" "$OUT" "$EV_FORMAT"

run_conf_cmd "$CMD_LINT"
assert_zero     "the lint gate passes on the unmodified tree"       "$RC" "$OUT"
assert_observed "lint reports 47 files on the unmodified tree"      "$BASE_LINT" "$OUT" "$EV_LINT"

run_conf_cmd "$CMD_TYPECHECK"
assert_zero     "the typecheck gate passes on the unmodified tree"  "$RC" "$OUT"
assert_observed "typecheck reports 8 files on the unmodified tree"  "$BASE_TYPECHECK" "$OUT" "$EV_TYPECHECK"
...
assert_narrowed_count "narrowing the format target to src reports 8, not 47" \
...
assert_narrowed_count "narrowing the lint target to src reports 8, not 47" \
...
assert_narrowed_count "narrowing the typecheck target to src/shared reports 5, not 8" \
...
assert_observed "AC-2: format counts the untracked file (47 -> 48)" "$((BASE_FORMAT + 1))" "$OUT" "$EV_FORMAT"

run_conf_cmd "$CMD_LINT"
assert_zero     "AC-4: the lint gate still passes with the untracked file present"   "$RC" "$OUT"
assert_observed "AC-4: lint counts the untracked file (47 -> 48)"   "$((BASE_LINT + 1))" "$OUT" "$EV_LINT"
...
assert_observed "typecheck counts the untracked file (8 -> 9)"      "$((BASE_TYPECHECK + 1))" "$OUT" "$EV_TYPECHECK"
...
assert_observed "format does not count the ignored file (still 47)"    "$BASE_FORMAT" "$OUT" "$EV_FORMAT"

run_conf_cmd "$CMD_LINT"
assert_zero     "the lint gate still passes with an ignored .luau file present"      "$RC" "$OUT"
assert_observed "lint does not count the ignored file (still 47)"      "$BASE_LINT" "$OUT" "$EV_LINT"

run_conf_cmd "$CMD_TYPECHECK"
assert_zero     "the typecheck gate still passes with an ignored .luau file present" "$RC" "$OUT"
assert_observed "typecheck does not count the ignored file (still 8)"  "$BASE_TYPECHECK" "$OUT" "$EV_TYPECHECK"
